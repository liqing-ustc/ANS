...
class HINT(Dataset):
    def __init__(self, split='train', exclude_symbols=None, exclude_zero_res=False, numSamples=None, randomSeed=None):
        ...
        if exclude_zero_res:
            self.dataset = [x for x in self.dataset if len(x['expr']) == 1 or x['res'] != 0]
